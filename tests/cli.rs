...
fn json_output_contains_security_score() {
    ...
    assert!(v.get("security_score").is_some(), "missing 'security_score' key");
}
...
fn json_security_score_is_in_range() {
    ...
    let v: serde_json::Value =
        serde_json::from_str(&String::from_utf8_lossy(&out.stdout)).unwrap();
    let score = v["security_score"].as_u64().expect("security_score must be a number");
    assert!(score <= 100, "security_score must be in [0, 100], got {score}");
}
...
fn json_sections_have_virtual_address_and_file_offset() {
    ...
    assert!(first.get("virtual_address").is_some(), "missing 'virtual_address' in section");
    assert!(first.get("file_offset").is_some(), "missing 'file_offset' in section");
}
...
fn json_section_permissions_have_rwx_fields() {
    ...
    for sec in sections {
        ...
        assert!(perms.get("write").is_some(), "missing 'write' in permissions");
        assert!(perms.get("execute").is_some(), "missing 'execute' in permissions");
    }
}
...
fn json_text_section_is_executable() {
    ...
    if let Some(text) = sections.iter().find(|s| s["name"] == ".text") {
        assert_eq!(text["permissions"]["execute"], true, ".text must be executable");
    }
}
...
fn json_dangerous_symbols_have_name_and_category() {
    ...
    for sym in syms {
        ...
        assert!(sym.get("category").is_some(), "dangerous symbol missing 'category'");
